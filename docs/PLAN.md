# Plan: Convert bird (Node) to Python

## Goals
- Port the CLI and library from `../bird` (Node/TypeScript) into a Python project in this repo.
...
## Discovery
1. Read `../bird/README.md`, `package.json`, and `src/**` to understand CLI commands, options, and outputs.
...
3. Review tests in `../bird/tests/**` to capture expected behavior and fixtures.
...
2. Use `src/` layout, e.g. `src/bird/` for library and `src/bird/cli.py` for CLI.
3. Add `tests/` with `pytest`, mirroring behavior in `../bird/tests/**`.
...
3. Port config parsing for JSON5 (`.birdrc.json5` and `~/.config/bird/config.json5`).
...
1. Define console script `bird` entry point in `pyproject.toml`.
